...
def cost(params):
    sqs : SquareSolver = SquareSolver(geometric_info, h, heat_source, base_temp,
                                  penalization, export_domain, export_solution)
...
def safe_evaluate(pool, params, timeout = 2):
    # Schedule the single execution in the pool
    future = pool.schedule(cost, args=(params,), timeout=timeout)
    
    try:
        return future.result()
    except TimeoutError:
        print("Cost evaluation timed out.")
        return float('inf')
    except ProcessExpired as e:
        print(f"Cost evaluation crashed (Segfault/Memory Error). Exit code: {e.exitcode}")
        return float('inf')
    except Exception as e:
        print(f"Cost evaluation failed: {e}")
        return float('inf')

def simmulated_annealing(initial_params, scales, initial_temp, cooling_rate, max_iter):
    # Initialization
    current_params = np.array(initial_params, dtype=float)

    with ProcessPool(max_workers=1, max_tasks=1) as pool:
        current_cost = safe_evaluate(pool, initial_params)
        ...
        for i in range(max_iter):
            ...
            neigh_cost = safe_evaluate(pool, neigh_params)
...
if __name__ == '__main__':
    ...
    best_params = simmulated_annealing(initial_ellipses, scales, 250, 0.98, 1000)

    print('Best parameters:')
    print(best_params.tolist())
    print(f'Objective: {cost(best_params)}')