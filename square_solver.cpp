...
SquareSolver::SquareSolver(DomainConfig Cfg, const double heat_sources_, const double base_temp_, const double penalization_, const bool export_mesh_) :
    cfg(Cfg), heat_sources(heat_sources_), base_temp(base_temp_), penalization(penalization_), export_mesh(export_mesh_), ambient(tag::Euclid, tag::of_dim, 2),
    north(tag::non_existent), south(tag::non_existent), sources(tag::non_existent), square_boundary(tag::non_existent){
...
double SquareSolver::solve(EllipseBundle& bundle){
    ...
    Mesh inner_boundary = bundle.manual_total_mesh();
